...
def procesar_fuente_voz(fuente: AnalisisPuntoFuente, absolute_path: str):
    ...
    for _, row in df_traf.iterrows():
        ...
        e = AnalisisPuntoEvento(
            ...
            raw_payload_json=json.dumps({str(k): (None if pd.isna(v) else str(v)) for k, v in row.to_dict().items()}, ensure_ascii=False),
        )
        db.session.add(e)
        count_eventos += 1

    fuente.upload_status = "PROCESSED"
    fuente.error_detail = None
    db.session.add(fuente)
    db.session.commit()
    return count_eventos
